...
class UIElement{
  public:
    ...
    */
    void setFocusListener(Focus* listener){parent_focus = listener;}
  ...
  protected:
    ...
    Identity id;
    Focus* parent_focus; //This is the focus oject that the element will listen to for its own updates
};
...
class MonoApp : public UIElement{
  public:
    MonoApp(Image8* unfocused, Image8* focused, int posx, int posy, bool isCentered):UIElement(unfocused->width, unfocused->height, posx, posy){
      centered = isCentered;
    ...
    void render() override;
  protected:
    Image8* unselected = nullptr;
    Image8* selected = nullptr;
    Image8* showing = nullptr;
};
